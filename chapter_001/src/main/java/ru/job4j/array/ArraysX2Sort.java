 ...
 * обратите внимание на строчу 104 последнее значение ПРЕДРЕШЕНО))))
 * я знал что вы меня об этом спросите т.к. я ещё ночью об  этом задумался и пол дня потратил на написание условий
 * выручило sout чтобы отслеживать что происходит)) когда был задан вопрос :
 * что будет если массивы разной длинны- ответ уже почти был готов) я только додумывал что делать с последним значением,
 * однако всё работает эксперементировал с разными масссивами и разной длины
 */
public class ArraysX2Sort {
    ...
    public static int[] ssortirovkaMassivavodin(int[]ar1, int[]ar2) {
        ...
        for (int i = 0; i < ar3.length - 1; i += 2) {
            ...
            ar3[ar3.length - 1] = ar1[ar1.length - 1] >= ar2[ar2.length - 1] ? ar1[ar1.length - 1] : ar2[ar2.length - 1];
        }
        return ar3;
    }
}
